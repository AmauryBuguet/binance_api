import 'binance_abstract.dart';

import 'classes.dart';
import 'enums.dart';

extension Websockets on BinanceAbstract {
  ...
  Stream<WsTrade> tradeStream({required String symbol}) {
    ...
    return channel.stream
        .map<Map>(toMap)
        .map<WsTrade>((e) => WsTrade.fromMap(e));
  }
  ...
  Stream<WsPartialOrderBook> partialOrderbookStream({
    ...
    final channel = subscribe(
        '${symbol.toLowerCase()}@depth$nbLevel' + (is100ms ? "@100ms" : ""));
    return channel.stream
        .map<Map>(toMap)
        .map<WsPartialOrderBook>((e) => WsPartialOrderBook.fromMap(e));
  }
